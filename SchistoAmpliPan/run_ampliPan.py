#!/Users/rnabunje/anaconda3/bin/python
...
def main():
    ...
    if args.mode == "genome_wide":
        if args.flag == "design_amplicons":
            return ampliPan.design_genome_wide_amplicons(snp_vcf=vcf, a_size=alen, exclude_bed=exclude,
                                                            out_file=out, pls=plr)
        elif args.flag == "design_primers":
            return ampliPan.design_primers(amplicons_file=file, a_size=alen, ref_seq=ref, out_file=out)
        elif args.flag == "space_amplicons":
            return ampliPan.space_amplicons(primers_file=file, ref_seq=ref, total_amplicons=amps, out_file=out)
        elif args.flag == "score_primer_compatibility":
            return ampliPan.score_primer_compatibility(primers_file=file, dg_threshold=dg, out_file=out)
        elif args.flag == "score_adapter_compatibility":
            return ampliPan.score_adapter_compatibility(primers_file=file, forward_ad=ad1,
                                                           reverse_ad=ad2, dg_threshold=dg, out_file=out)
        elif args.flag == "best_scoring_pairs":
            return ampliPan.best_scoring_pairs(scored_primers_file=file, out_file=out)
        elif args.flag == "add_tails":
            return ampliPan.add_tails(best_primers_file=file, leftprimer_tail=tail1, rightprimer_tail=tail2,
                                         out_file=out)
        else:
            amplicons = ampliPan.design_genome_wide_amplicons(snp_vcf=vcf, a_size=alen, exclude_bed=exclude,
                                                                 out_file=out, pls=plr)
            primers = ampliPan.design_primers(amplicons_file=amplicons, a_size=alen, ref_seq=ref, out_file=out)
            spaced_primers = ampliPan.space_amplicons(primers_file=primers, ref_seq=ref, total_amplicons=amps,
                                                         out_file=out)
            dimer_checked = ampliPan.score_primer_compatibility(primers_file=spaced_primers, dg_threshold=dg,
                                                                   out_file=out)
            adapter_compatible = ampliPan.score_adapter_compatibility(primers_file=dimer_checked, forward_ad=ad1,
                                                                         reverse_ad=ad2, dg_threshold=dg, out_file=out)
            best_primers = ampliPan.best_scoring_pairs(scored_primers_file=adapter_compatible, out_file=out)
            tailed_primers = ampliPan.add_tails(best_primers_file=best_primers, leftprimer_tail=tail1,
